...
class PathDescribeTest {
    ...
    fun `Test manual operation registration`() {
        ...
        for ((methodName, methodGetter) in methods) {
            ...
            plugin.registerOperation("/", io.ktor.http.HttpMethod(methodName)) {
                description = "Hello"
            }
            assertEquals("Hello", plugin.buildOpenApiDocument().paths["/"]?.methodGetter()?.description)
        }
    }
    ...
    fun `Unknown method is ignored`() {
        val plugin = TegralOpenApiKtor()
        plugin.registerOperation("/", io.ktor.http.HttpMethod("FOOBAR")) {
            description = "Hello"
        }
